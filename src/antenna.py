...
#
"""3GPP TR 38.811 antenna modeling"""
...
class AntennaElement:
    ...
    def access_bessel_function(self, index):
        """
        The aperture antenna uses the bessel function to calculate the gain. However, as the bessel function cannot be
        computed in graph mode, a sufficient number of values was precalculated and is read from during simulations.
        """
        return tf.gather(self.bessel_function_values, index)

    # tf function to remain functioning during graph execution.
    @tf.function
    def compute_bessel_function(self, input_values):
        """
        This code takes all input values and returns the bessel function value. This is done in recursion to support 
        different tensor shapes

        Inputs
        -------
        input_value:
            tensor of arbitrary shapes with elevation angles in [degree] and resolution of 0.1 degree

        Output
        ------
        bessel_value:
            tensor of same shape as input_value with the bessel functions result for every input value
        """
        if len(input_values.shape) == 1:
            # the bessel function saves the elevation angle in a resolution of 0.1 degree. Thus the access index is the elevation angle
            # in degrees times 10.
            bessel_values = tf.map_fn(fn = lambda elem : self.access_bessel_function(tf.cast(elem*10,tf.int32)), elems=input_values)
        elif len(input_values.shape) > 0:
            bessel_values = tf.map_fn(fn = lambda elem : self.compute_bessel_function(elem), elems=input_values)
    ...
    def _radiation_pattern_circular_aperture(self, theta, phi, a = 10):
        ...
        """
        bessel_file = files(models).joinpath("bessel.npy")

        with open(bessel_file, 'rb') as f:
            self._bessel_function_values = np.load(f)

        ka = a * 2 * math.pi
        r = tf.acos(tf.sin(theta) * tf.cos(phi))
        amplitude = tf.where(0 == r, tf.cast(0.0, tf.float32), 4 * tf.abs(self.compute_bessel_function(tf.sin(r)*ka) / (ka * tf.sin(r))) ** 2)
    ...
    #TODO There is a remaining bug with this implementation sometimes occuring. This bug needs to be understood and fixed in the future. 
    def _radiation_pattern_co_phased_DLp(self, theta, phi):
        ...
        phi_3db = 90
        Am = 25
        A_EV = -tf.math.minimum(12 * (theta - 90 / theta_3db), SLAv)
        A_EH = -tf.math.minimum(12 * (phi/phi_3db) ** 2, Am)
    ...
    @property
    def bessel_function_values(self):
        """
        Bessel function can not be computed when running in graph mode, thus sufficiently many values were
        calculated and stored beforehand in this variable which is accessed with an angular value between 
        [-90.0,90.0] instead of caluculating the function.
        """
        return self._bessel_function_values
