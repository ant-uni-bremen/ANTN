    ...
    def test_dl_s_band_too_high(self):
        ...
        with self.assertRaises(Exception) as context:
            channel_model = Urban(carrier_frequency=carrier_frequency,
                                                ...
                                                enable_pathloss=True,
                                                enable_shadow_fading=True)
        self.assertTrue("Carrier frequency in downlink must be either in S Band (2.17GHz-2.2GHz) or Ka Band (19.7GHz - 21.2GHz)" in str(context.exception))
    ...
    def test_ul_s_band_too_high(self):
        ...
        with self.assertRaises(Exception) as context:
            channel_model = Urban(carrier_frequency=carrier_frequency,
                                                ...
                                                enable_pathloss=True,
                                                enable_shadow_fading=True)
        self.assertTrue("Carrier frequency in uplink must be either in S Band (1.98GHz-2.01GHz) or Ka Band (29.5GHz - 30GHz)" in str(context.exception))
    ...
    def test_sat_too_low(self):
        ...
        with self.assertRaises(Exception) as context:
            channel_model = Urban(carrier_frequency=carrier_frequency,
                                                ...
                                                enable_pathloss=True,
                                                enable_shadow_fading=True)
            topology = utils.gen_single_sector_topology(batch_size=1, num_ut=1, scenario="urb", elevation_angle=elevation_angle, bs_height=500000)
            channel_model.set_topology(*topology)
        self.assertTrue("The base station height must be at least 600'000m and not more than 36'000'000m, as only satellites are currently supported" in str(context.exception))
